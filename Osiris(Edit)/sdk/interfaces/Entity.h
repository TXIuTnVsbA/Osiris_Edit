...
public:
    ...
    bool setupBones(matrix3x4* out, int maxBones, int boneMask, float currentTime, bool fixBoneMatrix = false) noexcept
    {
        if (fixBoneMatrix) {
    ...
    bool isVisible(const Vector& position = { }) noexcept
    {
        ...
        Trace trace;
        g_interfaces.engineTrace->traceRay({ localPlayer->getEyePosition(), position ? position : getBonePosition(8) }, 0x46004009, { localPlayer.get() }, trace);
    ...
    bool isEnemy() noexcept
    {
        ...
        return g_memory.isOtherEnemy(this, localPlayer.get());
    }
