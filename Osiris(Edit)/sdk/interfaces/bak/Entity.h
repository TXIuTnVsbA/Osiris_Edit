...
public:
    ...
    constexpr bool setupBones(matrix3x4* out, int maxBones, int boneMask, float currentTime,bool fixBoneMatrix =false) noexcept
    {
    ...
    {
        auto localPlayer = g_interfaces.entityList->getEntity(g_interfaces.engine->getLocalPlayer());
        static Trace trace;
        g_interfaces.engineTrace->traceRay({ localPlayer->getEyePosition(), position ? position : getBonePosition(8) }, 0x46004009, { localPlayer }, trace);
        return trace.entity == this || trace.fraction > 0.97f;
    }

    bool isEnemy() noexcept
    {
        return g_memory.isOtherEnemy(this, g_interfaces.entityList->getEntity(g_interfaces.engine->getLocalPlayer()));
    }
    ...
    NETVAR_OFFSET(crosshairId, "CCSPlayer", "m_bHasDefuser", 92, int)
